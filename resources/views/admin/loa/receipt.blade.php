...
<div id="modal-send-loa" class="no-print"
    ...
    <div style="background:#1e1e2e; color:#e0e0e0; border-radius:10px; width:100%; max-width:500px;
        ...
        @php
            ...
            $resendWaRoute = (!empty($isMarketingView) && $isMarketingView)
                ? route('marketing.loa-master.resend-wa', $submission)
                : route('admin.loa-master.resend-wa', $submission);
        @endphp
        ...
        <div style="display:flex; gap:10px; flex-wrap:wrap;">
            @if($noHp)
            <div style="flex:1; min-width:140px;">
                <form method="POST" action="{{ $resendWaRoute }}"
                      onsubmit="return confirm('Kirim LOA via WhatsApp ke {{ $submission->no_hp_penulis }} sekarang?');">
                    @csrf
                    <button type="submit"
            ...
            @if($emailPenulis)
            <div style="flex:1; min-width:140px;">
                <form method="POST" action="{{ $resendLoaRoute }}"
...
<script>
    ...
    _e.style.cssText = 'position:fixed;top:60px;right:20px;background:#c62828;color:#fff;padding:12px 20px;border-radius:8px;font-family:sans-serif;font-size:13px;z-index:9999;box-shadow:0 4px 12px rgba(0,0,0,.3);';
    _e.textContent = '⚠ {{ session("error") }}';
    document.body.appendChild(_e);
    setTimeout(function(){ _e.remove(); }, 5000);
</script>
@endif
@endif

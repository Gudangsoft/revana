...
@section('title', 'Edit Kategori - ' . $appSettings['app_name'])
            ...
            <div class="card-body">
                <form action="{{ route('admin.kategoris.update', $kategori) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3">{{ old('description', $kategori->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>

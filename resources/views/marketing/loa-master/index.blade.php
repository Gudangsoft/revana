...
.filter-chip:hover, .filter-chip-auto:hover { border-color: #11998e; color: #11998e; }
.filter-chip.active  { background: #11998e; border-color: #11998e; color: #fff; }
.filter-chip-auto.active { background: #16a34a; border-color: #16a34a; color: #fff; }
                ...
                <div class="card border-0 shadow-sm h-100" style="cursor:pointer;" onclick="setFilter('all','all')">
                    <div class="card-body py-3">
                        <div class="d-flex align-items-center gap-3">
                            <div style="width:42px;height:42px;border-radius:10px;background:#e8f5e9;display:flex;align-items:center;justify-content:center;">
                                <i class="bi bi-journals" style="color:#11998e;font-size:1.2rem;"></i>
                            </div>
                            <div>
                                <div style="font-size:1.5rem;font-weight:800;line-height:1;">{{ $stats['total'] }}</div>
                                <div class="text-muted" style="font-size:.78rem;">Jurnal Saya</div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body py-3">
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <div>
                                ...
                                <div class="text-muted" style="font-size:.78rem;">LOA Otomatis</div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="card shadow-sm">
            <div class="card-header py-3">
                <div class="row g-2 align-items-center">
                    <div class="col-12 col-md-auto me-auto">
                        <span class="fw-semibold">
                            ...
                            Master LOA — Jurnal yang Anda Kelola
                        </span>
                    </div>
                ...
                <div class="d-flex flex-wrap gap-2 mt-3 align-items-center">
                    ...
                    <button class="btn btn-sm filter-chip-auto" data-filter-auto="manual">Manual</button>
                    ...
                    <table class="table table-hover align-middle mb-0" id="loaTable">
                        ...
                        <tbody>
                            ...
                            <tr class="loa-row"
                                ...
                                data-search="{{ strtolower($j->nama_jurnal . ' ' . $j->kode_singkat . ' ' . $j->e_issn) }}">
                                ...
                                <td class="text-center">
                                    <div class="d-flex gap-1 justify-content-center">
                                        <a href="{{ route('marketing.loa-master.edit', $j) }}"
                                           class="btn btn-sm btn-outline-primary" title="Setting LOA">
                                            <i class="bi bi-gear"></i> Setting
                                        </a>
                                        <a href="{{ route('marketing.loa-master.preview', $j) }}"
                            ...
                            <tr>
                                <td colspan="7" class="text-center py-5 text-muted">
                                    ...
                                    Belum ada jurnal. Mulai dengan submit artikel ke jurnal.
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
...
(function () {
    ...
    btnClear.addEventListener('click', function () {
        searchInp.value = ''; activeSearch = '';
        btnClear.style.display = 'none';
        applyFilters(); searchInp.focus();
    });
    ...
    function applyFilters() {
        ...
        rowCountEl.textContent = visible < total ? visible + ' dari ' + total + ' jurnal' : total + ' jurnal';
        emptyEl.style.display  = (visible === 0 && rows.length > 0) ? '' : 'none';
        btnReset.style.display = (activeComplete !== 'all' || activeAuto !== 'all' || activeSearch !== '') ? '' : 'none';
    }

    window.resetAllFilters = function () {
        activeComplete = 'all'; activeAuto = 'all'; activeSearch = '';
        searchInp.value = ''; btnClear.style.display = 'none';
        document.querySelectorAll('.filter-chip').forEach(function (b) { b.classList.toggle('active', b.dataset.filterComplete === 'all'); });
        document.querySelectorAll('.filter-chip-auto').forEach(function (b) { b.classList.toggle('active', b.dataset.filterAuto === 'all'); });
        applyFilters();
    };

    window.setFilter = function (complete, auto) {
        activeComplete = complete; activeAuto = auto; activeSearch = '';
        searchInp.value = ''; btnClear.style.display = 'none';
        document.querySelectorAll('.filter-chip').forEach(function (b) { b.classList.toggle('active', b.dataset.filterComplete === complete); });
        document.querySelectorAll('.filter-chip-auto').forEach(function (b) { b.classList.toggle('active', b.dataset.filterAuto === auto); });
        applyFilters();

...
<div class="row">
    <div class="col-md-10 mx-auto">
        ...
        <nav class="mb-3" style="font-size:.85rem;">
            <a href="{{ route('marketing.loa-master.index') }}" class="text-decoration-none">Master LOA</a>
            <span class="text-muted mx-1">/</span>
            <span class="text-muted">{{ $journal->nama_jurnal }}</span>
        </nav>

        <form action="{{ route('marketing.loa-master.update', $journal) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="d-flex justify-content-between">
                <a href="{{ route('marketing.loa-master.index') }}" class="btn btn-secondary">
...
(function () {
    function syncColor(picker, text, target) {
        picker.addEventListener('input', function () { text.value = this.value; target.style.background = this.value; });
        text.addEventListener('change', function () { picker.value = this.value; target.style.background = this.value; });
    }
    ...
    if (inpIssn && prevIssn) {
        inpIssn.addEventListener('input', function () { prevIssn.textContent = this.value; });
    }

    var sw = document.getElementById('swAutoSend');
    var swLbl = document.getElementById('swLabel');
    if (sw && swLbl) {
        sw.addEventListener('change', function () { swLbl.textContent = this.checked ? 'Aktif' : 'Non-aktif'; });
    }

    function filePreview(inputId, previewWrapId, previewImgId) {
    ...
    filePreview('inpAccLogo',   'accLogoPreview',   'accLogoPreviewImg');
    filePreview('inpHeaderImg', 'headerImgPreview', 'headerImgPreviewImg');
    filePreview('inpFooterImg', 'footerImgPreview', 'footerImgPreviewImg');

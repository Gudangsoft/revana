...
{{-- Tombol WhatsApp melayang untuk menghubungi admin.
     Nomor diatur admin di /admin/settings ("Nomor WhatsApp Admin"). --}}
@php
    ...
    $waGreeting = 'Halo Admin, saya ' . ($user->name ?? 'reviewer') . ' (reviewer). Saya ingin bertanya mengenai ';
@endphp
@if($adminWa !== '')
...
<style>
    .reviewer-wa-float {
        ...
        overflow: hidden;
        white-space: nowrap;
        transition: box-shadow .2s ease, transform .2s ease, padding .25s ease;
    ...
    @media (max-width: 576px) {
        .reviewer-wa-float { right: 16px; bottom: 16px; height: 54px; padding: 0 15px; }
        .reviewer-wa-float i { font-size: 1.55rem; }
    }
</style>
@endif
